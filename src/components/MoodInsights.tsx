...
export const MoodInsights = ({ moodData }: MoodInsightsProps) => {
  ...
  const calculateInsights = () => {
    ...
    const sortedByMood = [...moodData].sort((a, b) => parseInt(b.mood_scale) - parseInt(a.mood_scale));
    ...
    const weeklyAverage = last7Days.length > 0 
      ? last7Days.reduce((sum, entry) => sum + parseInt(entry.mood_scale), 0) / last7Days.length
      : 0;
  ...
  const calculateLongestStreak = (data: any[]) => {
    ...
    for (let i = 0; i < data.length; i++) {
      if (parseInt(data[i].mood_scale) >= 4) {
        currentStreak++;
  ...
  const calculateTrend = (data: any[]) => {
    ...
    const firstAvg = firstWeek.reduce((sum, entry) => sum + parseInt(entry.mood_scale), 0) / firstWeek.length;
    const lastAvg = lastWeek.reduce((sum, entry) => sum + parseInt(entry.mood_scale), 0) / lastWeek.length;
